...
for j,salt in enumerate(salts):
    ...
    for k,multiplier in enumerate(multipliers):
        ...
        for i,thickness in enumerate(multiplier_thicknesses):
                directory = '{}/{}/{}_{}cm'.format(model_directory, salt, multiplier, thickness)
                ...
                tbrs[salt][multiplier][i,0] = t_tally.get_reshaped_data(value='mean').squeeze()/source_strength
                tbrs[salt][multiplier][i,1] = t_tally.get_reshaped_data(value='std_dev').squeeze()/source_strength
                print(directory)
...
ax.set_xlabel('Multiplier Thickness')
ax.set_ylabel('TBR')
ax.legend()
fig.savefig('{}/libra_pie_multiplier_scan.png'.format(model_directory), dpi=200)
