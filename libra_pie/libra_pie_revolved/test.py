...
## Low Carbon Steel (1008) from PNNL Materials Compendium
steel = openmc.Material(name='steel')
steel.set_density('g/cc', 7.872)
steel.add_element('C',  0.0010, 'wo')
steel.add_element('Mn', 0.0050, 'wo')
steel.add_element('P',  0.0004, 'wo')
steel.add_element('S',  0.0005, 'wo')
steel.add_element('Fe', 0.9931, 'wo')
...
shield_bot_plane_1 = openmc.ZPlane(shield_bot_plane_2.z0 - shield_thickness)

shield_in_cyl = openmc.ZCylinder(r=outer_cyl_4.r+10)
shield_out_cyl = openmc.ZCylinder(r=shield_in_cyl.r+shield_thickness)
shield_top_plane_1 = openmc.ZPlane(salt_gas_tube_top_plane.z0)
shield_top_plane_2 = openmc.ZPlane(shield_top_plane_1.z0 + shield_thickness)
...
libra_bot_plane.boundary_type = 'vacuum'
libra_top_plane.boundary_type = 'vacuum'
libra_out_cyl.boundary_type = 'vacuum'
...
salt_gas_fill_reg = (-salt_gas_tube_1_cyl_1 & +tank_top_cover_plane_1 & -salt_gas_tube_top_plane) \
                  | (-salt_gas_tube_2_cyl_1 & +tank_top_cover_plane_1 & -salt_gas_tube_top_plane)
...
top_air_reg = top_air_1_reg | top_air_2_reg_1 | top_air_2_reg_2 \
            | top_air_2_reg_hor | top_air_2_reg_ver
...
for multiplier_thickness in multiplier_thicknesses:
    ...
    inner_air_reg = -inner_cyl_1 & -salt_gas_tube_top_plane & +shield_bot_plane_1 & +x_plane_1 & +y_plane_1 \
                    & ~multiplier_reg
    ...
    libra_outside_air_cell = openmc.Cell(region=libra_outside_air_reg, fill=air, name='LIBRA Outside Air')
        # void region
    support_plate_reg = +inner_cyl_1 & -outer_cyl_4 & +support_plate_bot_plane & -z_plane_1 & +x_plane_1 & +y_plane_1
    support_plate_cell = openmc.Cell(region=support_plate_reg, fill=steel, name='Support Plate')

    libra_insulation_reg = (+outer_cyl_4 & -shield_in_cyl & +support_plate_bot_plane & -shield_top_plane_1 & +x_plane_1 & +y_plane_1) \
    ...
    libra_shielding_cell = openmc.Cell(region=libra_shielding_reg, fill=BPE, name='LIBRA Shield')
    ...
    libra_quarter_1_cells = [out_wall_cell, wall_gap_cell, inner_wall_cell,
        ...
        multiplier_cell, inner_air_cell,
        libra_outside_air_cell,
        support_plate_cell, libra_insulation_cell, libra_shielding_cell]
    libra_quarter_1_universe = openmc.Universe(cells=libra_quarter_1_cells)
    libra_quarter_1_cell = openmc.Cell(region=libra_quarter_1_reg, fill=libra_quarter_1_universe, name='Quad 1')

    libra_quarter_2_reg = libra_quarter_1_reg.rotate((0,0,90))
    libra_quarter_2_cell = openmc.Cell(region=libra_quarter_2_reg, fill=libra_quarter_1_universe, name='Quad 2')
    libra_quarter_2_cell.rotation = [0.0, 0.0, 90.0]

    libra_quarter_3_reg = libra_quarter_1_reg.rotate((0,0,180))
    libra_quarter_3_cell = openmc.Cell(region=libra_quarter_3_reg, fill=libra_quarter_1_universe, name='Quad 3')
    libra_quarter_3_cell.rotation = [0.0, 0.0, 180.0]

    libra_quarter_4_reg = libra_quarter_1_reg.rotate((0,0,270))
    libra_quarter_4_cell = openmc.Cell(region=libra_quarter_4_reg, fill=libra_quarter_1_universe, name='Quad 4')
    ...
    cells = [libra_quarter_1_cell, libra_quarter_2_cell, libra_quarter_3_cell, libra_quarter_4_cell] #,
            # void_cell]
    # cells = [libra_quarter_1_cell]
    ...
    tallies = openmc.Tallies([t_tally, multiplier_n2n_tally])
