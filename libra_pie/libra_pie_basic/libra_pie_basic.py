...
multiplier_thickness = 6
starting_radius_1 = 10.0

# salt_heights = [98, 94, 90, 86, 82, 78, 74, 70]
...
reflector_th_dict = {}
for reflector_material in reflector_materials:
    if reflector_material.get_mass_density() > 3:
        reflector_th_dict[reflector_material] = [0, 2, 4, 6, 8, 10]
    else:
        reflector_th_dict[reflector_material] = [0, 5, 10, 15, 20, 25]

multiplier_material = beryllium

tbrs = np.zeros((len(salt_materials), len(reflector_materials), len(reflector_th_dict[graphite])))
...
# These radii are based on the original outer radius from the 9/25/2024 CAD model
...
libra_bot_plane = support_plate_bot_plane
libra_top_plane = salt_gas_tube_top_plane
libra_out_cyl = outer_cyl_4
####### Regions and Cells ###############
...
top_air_2_reg_2 = +outer_cyl_1 & -outer_cyl_4 \
                & +y_plane_4 & +x_plane_4 \
                & +double_wall_top_plane_2 & -salt_gas_tube_top_plane
top_air_3_reg = -outer_cyl_1 & +tank_top_cover_plane_1 & -salt_gas_tube_top_plane
...
support_plate_reg = +inner_cyl_1 & -outer_cyl_4 & +support_plate_bot_plane & -z_plane_1
support_plate_cell = openmc.Cell(region=support_plate_reg, fill=steel_lowC, name='Support Plate')



    # void region
libra_quarter_1_reg = -libra_out_cyl & +inner_cyl_1 & +libra_bot_plane & -libra_top_plane & +x_plane_1 & +y_plane_1
...
outside_libra_quarter_reg = -libra_out_cyl & +libra_bot_plane & -libra_top_plane & ~libra_quarter_1_reg & ~gen_1_region
outside_libra_quarter_cell = openmc.Cell(region=outside_libra_quarter_reg, fill=air, name='Outside LIBRA Quarter Air')

libra_universe = openmc.Universe(cells=[libra_quarter_1_cell, gen_1_cell, outside_libra_quarter_cell])
