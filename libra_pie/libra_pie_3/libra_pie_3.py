...
reflector_materials = [graphite, lead, beryllium, firebrick, HDPE, SS304, steel_lowC, zirconia, tungsten]
...
for salt_material in salt_materials:
    ...
    multiplier_top_plane = openmc.ZPlane(dt_generator_top_plane.z0 + multiplier_thickness)

    multiplier_reg = (+multiplier_inner_cyl & -multiplier_outer_cyl & +multiplier_bot_plane & -dt_generator_top_plane) \
                    | (-multiplier_outer_cyl & +dt_generator_top_plane & -multiplier_top_plane)
    ...
    for reflector_material in reflector_materials:
        for reflector_thickness in reflector_thicknesses:
            # reflector surfaces
            ...
            tallies = openmc.Tallies([t_tally, t_li_tally, multiplier_n2n_tally])

            if __name__ == '__main__':
                salt_directory = salt_material.name
                reflector_directory = '{}_{}cm'.format(reflector_material.name, reflector_thickness)
                ...
                if not os.path.isfile('volume_1.h5'):
