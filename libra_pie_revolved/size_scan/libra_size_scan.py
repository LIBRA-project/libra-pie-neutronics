...
# multiplier_thickness = 2.54 * 2
multiplier_thicknesses = [5.08]
# starting_radius_1 = 10.0

# salt_heights = [80, 85, 90, 95, 100, 105, 110, 120]
salt_heights = [90, 110, 150]
# salt_heights = [80]
# salt_thicknesses = [30, 32, 34, 36, 38, 40, 42, 44, 46, 48, 50]
salt_thicknesses = [36, 48, 60, 70, 80, 100, 120]
# salt_thicknesses = [36]

reflector_thicknesses = [20]

# Model: v2
#       v2 model has a fully enveloped reflector including at the bottom of the generator and a central tank salt height 
#       and a central tank salt height 5 cm above the quadrant salt plane
model_designator = 'v2'
...
def get_cllif_density(temp, LiCl_frac=0.695, cl37_enr=0.2424):
    ...
    """ Calculates ClLiF density (g/cc) given a temperature in Celsius
    and the LiCl molar fraction (LiCl_frac) and the Cl-37 enrichment (cl37_enr)
    by calculating the molar volume of LiCl and LiF from published correlations, 
    ...
    # M_LiCl = M_Cl35 * (1 - 0.2424) + M_Cl37 * 0.2424 + M_Li
...
def get_melting_temp(LiCl_frac):
    LiF_frac = 1 - LiCl_frac
    melt_T_data = np.array([[0.027467772, 600],
                   [0.173391029, 550],
                   [0.305, 500],
                   [0.370815637, 550],
                   [0.448068744, 600],
    ...
    if LiF_frac==0.305:
        ...
        coeffs = np.polyfit(melt_T_data[0:3,0],
                            melt_T_data[0:3,1],
                            2)
        melting_temp = np.polyval(coeffs, LiF_frac)
    elif LiF_frac > 0.305:
        coeffs = np.polyfit(melt_T_data[2:,0],
                            melt_T_data[2:,1],
                            2)
        melting_temp = np.polyval(coeffs, LiF_frac)
...
def get_flipb_density(temperature, PbF2_frac=0.6):
    """ Calculates FLiPb density (g/cc) given a temperature in Celsius
    and the PbF2 molar fraction (PbF2_frac)
    by calculating the molar volume of PbF2 and LiF from published correlations."""


    # Calculate LiCl density
    # Source: https://doi.org/10.1016/S0022-3115(98)00237-2
    rho_PbF2 = 4.48 - 9.61e-4 * (temperature - 273.15)
    ...
    k_LiF = 0.000490
    t_m_LiF = 848.2
    rho_LiF = rho_m_LiF - k_LiF*(temperature - t_m_LiF)

    ## Calculate molar volume
    # Molar Mass of LiCl
    M_PbF2 = 245.2 #g/mol
    V_PbF2 = M_PbF2/rho_PbF2

    M_LiF = 25.939 #g/mol
    V_LiF = M_LiF/rho_LiF

    # molar volume of mixture
    V_m = PbF2_frac*V_PbF2 + (1-PbF2_frac)*V_LiF
    # molar mass of mixture
    M_m = PbF2_frac*M_PbF2 + (1-PbF2_frac)*M_LiF

    density_flipb = M_m/V_m
    return density_flipb
...
# lif-licl - natural - pure - eutectic
...
# lif-licl - natural - pure - eutectic - at room temperature
...
# 50% lif - 50% licl - natural - pure
cllif_50 = openmc.Material(name='ClLiF_50')
cllif_50.add_element('F', .5*.500, 'ao')
cllif_50.add_element('Li', .5*.500 + .5*.500, 'ao')
cllif_50.add_element('Cl', .5*.500, 'ao')
cllif_50.set_density('g/cm3', get_cllif_density(700, LiCl_frac=0.5))

# FLiPb eutectic (60% PbF2, 40% LiF) (melting point at ~577 C)
PbF2_frac = 0.6
flipb = openmc.Material(name='FLiPb')
flipb.add_element('F', 0.5 * (1 - PbF2_frac) + (2/3) * (PbF2_frac), 'ao')
flipb.add_element('Li', 0.5 * (1 - PbF2_frac), 'ao')
flipb.add_element('Pb', (1/3) * PbF2_frac, 'ao')
flipb.set_density('g/cm3', get_flipb_density(700, PbF2_frac=PbF2_frac))
...
salt_materials = [flipb, cllif_nat, cllif_cold]
...
tbrs = np.zeros((len(salt_materials), len(reflector_thicknesses), len(multiplier_thicknesses), len(salt_heights), len(salt_thicknesses)))
salt_volumes = np.zeros((len(salt_materials), len(reflector_thicknesses), len(multiplier_thicknesses), len(salt_heights), len(salt_thicknesses)))
...
insulation_thickness = 6
starting_radius_1 = 2.54 * 5.25
...
for i,salt_material in enumerate(salt_materials):
    for r,reflector_thickness in enumerate(reflector_thicknesses):
        for m,multiplier_thickness in enumerate(multiplier_thicknesses):
            for j,salt_height in enumerate(salt_heights):
                for k,salt_thickness in enumerate(salt_thicknesses):
                    if gen_out_cyl.r + 2 + multiplier_thickness > starting_radius_1:
                        inner_cyl_1 = openmc.ZCylinder(r=gen_out_cyl.r + 2 + multiplier_thickness)
                    else:
                        inner_cyl_1 = openmc.ZCylinder(r=starting_radius_1)
                    ...
                    # Adding 10 cm salt and tank height to center tank to enable greater salt thickness in center tank while reducing overall volume
                    center_tank_top_plane_1 = openmc.ZPlane(tank_top_cover_plane_1.z0 + 10.0)
                    ...
                    if multiplier_thickness < 8:
                        ...
                        multiplier_top_th = 8
                    multiplier_top_plane_2 = openmc.ZPlane(source_z_point + 13 + multiplier_thickness)
                    multiplier_top_plane_1 = openmc.ZPlane(source_z_point + 13)
                    dt_gen_top_plane = openmc.ZPlane(source_z_point + 12)
                    multiplier_bot_plane = openmc.ZPlane(source_z_point - 20)

                    center_tank_bot_plane_1 = openmc.ZPlane(multiplier_top_plane_2.z0 + 2.0)
                    ...
                    max_cover_z = np.max([tank_top_cover_plane_2.z0, double_wall_top_plane_2.z0, center_tank_top_plane_2.z0])
                    salt_gas_tube_top_plane = openmc.ZPlane(max_cover_z + 5)
                    ...
                    boundary_top_plane = openmc.ZPlane(300, boundary_type='vacuum')
                    ...
                    multiplier_inner_cyl = openmc.ZCylinder(r=gen_out_cyl.r + 1.0)
                    multiplier_outer_cyl = openmc.ZCylinder(r=multiplier_inner_cyl.r + multiplier_thickness)
                    ...
                    # End inner air on inside bottom surface of shield
                    # inner_air_reg = -inner_cyl_1 & -center_tank_bot_plane_1 & +shield_bot_plane_1 & +x_plane_1 & +y_plane_1 \
                    #                 & ~multiplier_reg & ~gen_1_region

                    # End inner air on outside bottom surface of shield
                    inner_air_reg = -inner_cyl_1 & -center_tank_bot_plane_1 & +shield_bot_plane_2 & +x_plane_1 & +y_plane_1 \
                                    & ~multiplier_reg & ~gen_1_region
                    ...
                    libra_shielding_reg = (-shield_out_cyl & +shield_top_plane_1 & -shield_top_plane_2) \
                                        | (-shield_out_cyl & +shield_in_cyl & +shield_bot_plane_2 & -shield_top_plane_1) \
                                        | (-shield_out_cyl & +shield_bot_plane_1 & -shield_bot_plane_2 & ~gen_1_region)
                    ...
                    if __name__ == '__main__':
                        ...
                        if model_designator != '':
                            multiplier_directory += '_' + model_designator
                        ...
                        tbrs[i,r,m,j,k] = np.sum(t_tally.get_reshaped_data(value='mean').squeeze())

                        salt_volumes[i,r,m,j,k] = salt_volume
...
tbr_levs = [0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2]

for i,salt_mat in enumerate(salt_materials):
    salt_masses = salt_mat.get_mass_density() * salt_volumes[i,:,:,:] / 1000 # in kg
    for r,reflector_thickness in enumerate(reflector_thicknesses):
        for m,multiplier_thickness in enumerate(multiplier_thicknesses):
            fig, ax = plt.subplots(figsize=[6,5])
            cntr = ax.contourf(xmesh, ymesh, tbrs[i,r,m,:,:], levels=tbr_levs, cmap='plasma', alpha=1.0)
            conlines = ax.contour(xmesh, ymesh, tbrs[i,r,m,:,:], levels=tbr_levs,
                                colors=['black'])
            ...
            # # Plot volume
            # vol_conlines = ax.contour(xmesh, ymesh, salt_volumes[i,r,m,:,:], levels=5, colors=['white'])
            # ax.clabel(vol_conlines, vol_conlines.levels, inline=True, fmt='vol=%.1e')

            # Plot mass contours
            mass_conlines = ax.contour(xmesh, ymesh, salt_masses[r,m,:,:], levels=5, colors=['white'])
            ax.clabel(mass_conlines, mass_conlines.levels, inline=True, fmt='%.1e kg')
            ...
            ax.set_title('{}, {} cm {}, {} cm {}'.format(salt_mat.name,
                                                        ...
                                                        reflector_thickness,
                                                        reflector_material.name))
            fig.savefig('{}_{}_size_scan.png'.format(salt_mat.name, multiplier_directory))
