...
salt_heights = [110, 106, 102, 98, 94, 90, 86, 82, 78, 74, 70]
...
tbrs = np.zeros((len(salt_materials), len(salt_heights)))
...
for j,salt_material in enumerate(salt_materials):
    for i,salt_height in enumerate(salt_heights):
        ...
        center_tank_top_plane_1 = openmc.ZPlane(112.71 + z_plane_4.z0)
        ...
        # Quadrant 1 initial salt volume
        initial_salt_vol = (112.71 - salt_headspace)*((52.39-4*libra_wall_th)**2 - inner_cyl_4.r**2) * np.pi/4
        new_salt_outer_radius = np.sqrt(initial_salt_vol/(np.pi/4 * salt_height) + inner_cyl_4.r**2)
        print('old outer radius: {:.2f} cm, new outer_radius: {:.2f} cm'.format(52.39-4*libra_wall_th, 
                                                                                new_salt_outer_radius))
        ...
        if __name__ == '__main__':
            ...
            height_directory = 'height_{:.0f}cm'.format(salt_height)
            if not os.path.isdir(salt_directory):
                os.mkdir(salt_directory)
            directory = salt_directory + '/' + height_directory
            ...
            tbrs[j,i] = np.sum(t_tally.get_reshaped_data(value='mean').squeeze())
            os.chdir('..')
            os.chdir('..')
...
for j,salt_mat in enumerate(salt_materials):
    ax.plot(salt_heights, tbrs[j,:], label=salt_mat.name)
ax.set_xlabel('Salt Height [cm]')
ax.set_ylabel('TBR')
ax.legend()
