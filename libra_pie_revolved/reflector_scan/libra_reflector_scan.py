...
for i,salt_material in enumerate(salt_materials):
    for j,reflector_material in enumerate(reflector_materials):
        reflector_thicknesses = reflector_th_dict[reflector_material]
        for k,reflector_thickness in enumerate(reflector_thicknesses):
            ...
            if __name__ == '__main__':
                ...
                reflector_directory = '{}_{:.0f}cm'.format(reflector_material.name, reflector_thickness)
                ...
                tbrs[i,j,k] = np.sum(t_tally.get_reshaped_data(value='mean').squeeze())
                os.chdir('..')
                os.chdir('..')
...
for i,salt_mat in enumerate(salt_materials):
    ...
    for j,reflector_material in enumerate(reflector_materials):
        reflector_thicknesses = reflector_th_dict[reflector_material]
        ax.plot(reflector_thicknesses, tbrs[i,j,:], label=reflector_material.name)
    ax.set_xlabel('Reflector Thickness [cm]')
    ax.set_ylabel('TBR')
    ax.set_title(salt_mat.name)
    ax.legend()
    fig.savefig('libra_{}_reflector_scan.png'.format(salt_mat.name))
