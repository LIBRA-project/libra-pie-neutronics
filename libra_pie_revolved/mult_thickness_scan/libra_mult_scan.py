...
multiplier_thicknesses = [0, 2, 4, 6, 8, 10, 12, 14, 16]
...
tbrs = np.zeros((len(salt_materials), len(multiplier_thicknesses)))
...
for j,salt_material in enumerate(salt_materials):
    for i,multiplier_thickness in enumerate(multiplier_thicknesses):
        ...
        if multiplier_thickness < 8:
            ...
            multiplier_top_th = 8
        multiplier_top_plane = openmc.ZPlane(source_z_point + 12 + multiplier_thickness)
        dt_gen_top_plane = openmc.ZPlane(source_z_point + 12)
        multiplier_bot_plane = openmc.ZPlane(source_z_point - 15)

        center_tank_bot_plane_1 = openmc.ZPlane(multiplier_top_plane.z0 + 2)
        ...
        top_air_2_reg_2 = +outer_cyl_1 & -outer_cyl_4 \
                        & +y_plane_4 & +x_plane_4 \
                        & +double_wall_top_plane_2 & -salt_gas_tube_top_plane
        top_air_3_reg = -outer_cyl_1 & +tank_top_cover_plane_2 & -salt_gas_tube_top_plane
        ...
        multiplier_reg = (+multiplier_inner_cyl & -multiplier_outer_cyl & +multiplier_bot_plane & -dt_gen_top_plane) \
                        | (-multiplier_outer_cyl & +dt_gen_top_plane & -multiplier_top_plane)
        multiplier_cell = openmc.Cell(region=multiplier_reg, fill=multiplier_material, name='Multiplier')

        ### Center Tank Regions and Cells
        center_tank_out_gap_reg = -inner_cyl_1 & +center_tank_bot_plane_1 & -tank_top_cover_plane_2 & +center_tank_cyl_4
        center_tank_out_gap_cell = openmc.Cell(region=center_tank_out_gap_reg, fill=air, name='Center Tank Outer Gap')

        center_tank_out_wall_reg = (-center_tank_cyl_4 & +tank_top_cover_plane_1 & -tank_top_cover_plane_2) \
                                 | (-center_tank_cyl_4 & +center_tank_cyl_3 & +center_tank_bot_plane_2 & -tank_top_cover_plane_1) \
        ...
        center_tank_in_gap_reg = (-center_tank_cyl_3 & +center_tank_cyl_2 & +center_tank_bot_plane_3 & -tank_top_cover_plane_1) \
        ...
        center_tank_in_wall_reg = (-center_tank_cyl_2 & +center_tank_cyl_1 & +center_tank_bot_plane_3 & -tank_top_cover_plane_1) \
        ...
        center_tank_salt_reg = (-center_tank_cyl_1 & +center_tank_bot_plane_4 & -salt_top_plane)

        center_tank_salt_cell = openmc.Cell(region=center_tank_salt_reg, fill=salt_material, name='center tank salt')

        center_tank_headspace_reg = -center_tank_cyl_1 & +salt_top_plane & -tank_top_cover_plane_1
        ...
        lead_shield_cell = openmc.Cell(region=lead_shield_reg, fill=lead, name='lead shield')
        ...
        if __name__ == '__main__':
            ...
            reflector_directory = '{}_{}cm'.format(multiplier_material.name, multiplier_thickness)
            ...
            tbrs[j,i] = t_tally.get_reshaped_data(value='mean').squeeze()
            os.chdir('..')
            os.chdir('..')
...
for j,salt_mat in enumerate(salt_materials):
    ax.plot(multiplier_thicknesses, tbrs[j,:], label=salt_mat.name)
ax.set_xlabel('Beryllium Thickness')
ax.set_ylabel('TBR')
ax.legend()
fig.savefig('libra_mult_thickness_scan.png')
